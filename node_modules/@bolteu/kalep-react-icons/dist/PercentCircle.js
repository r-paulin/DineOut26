import * as React from "react";
const sizes = {
    xs: 16,
    sm: 20,
    md: 20,
    lg: 24,
    xl: 36,
};
function SvgPercentCircle(props) {
    return (React.createElement("svg", Object.assign({ width: sizes[props.size || "lg"], height: sizes[props.size || "lg"], viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg" }, props),
        React.createElement("path", { d: "M8.7918 7.625C8.14739 7.625 7.625 8.14739 7.625 8.7918C7.625 9.43621 8.1474 9.9586 8.7918 9.9586C9.43621 9.9586 9.9586 9.43621 9.9586 8.7918C9.9586 8.1474 9.43621 7.625 8.7918 7.625Z", fill: "currentColor" }),
        React.createElement("path", { d: "M14.0414 15.2082C14.0414 14.5638 14.5638 14.0414 15.2082 14.0414C15.8526 14.0414 16.375 14.5638 16.375 15.2082C16.375 15.8526 15.8526 16.375 15.2082 16.375C14.5638 16.375 14.0414 15.8526 14.0414 15.2082Z", fill: "currentColor" }),
        React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M1 12C1 5.92487 5.92487 1 12 1C18.0751 1 23 5.92487 23 12C23 18.0751 18.0751 23 12 23C5.92487 23 1 18.0751 1 12ZM16.1251 6.63745C16.4668 6.29574 17.0208 6.29574 17.3626 6.63745C17.7043 6.97916 17.7043 7.53317 17.3626 7.87488L7.87491 17.3625C7.5332 17.7042 6.97918 17.7042 6.63747 17.3625C6.29576 17.0208 6.29576 16.4668 6.63747 16.1251L16.1251 6.63745ZM5.875 8.7918C5.875 7.1809 7.1809 5.875 8.7918 5.875C10.4027 5.875 11.7086 7.1809 11.7086 8.7918C11.7086 10.4027 10.4027 11.7086 8.7918 11.7086C7.1809 11.7086 5.875 10.4027 5.875 8.7918ZM15.2082 12.2914C13.5973 12.2914 12.2914 13.5973 12.2914 15.2082C12.2914 16.8191 13.5973 18.125 15.2082 18.125C16.8191 18.125 18.125 16.8191 18.125 15.2082C18.125 13.5973 16.8191 12.2914 15.2082 12.2914Z", fill: "currentColor" })));
}
export default SvgPercentCircle;
