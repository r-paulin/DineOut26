...
describe("computeOfferCardCampaign", () => {
  it("picks highest discount; all-day primary", () => {
    const offers: RestaurantTimedOffer[] = [
      { discountPercent: 30, window: { kind: "all-day" } },
      { discountPercent: 20, window: { kind: "range", start: "12:00", end: "15:00" } },
    ]
  ...
  it("Neiburgs-like: primary -30% evening", () => {
    const offers: RestaurantTimedOffer[] = [
      { discountPercent: 30, window: { kind: "range", start: "19:00", end: "23:00" } },
      { discountPercent: 25, window: { kind: "range", start: "11:00", end: "14:00" } },
  ...
  it("tie-break keeps earlier offer when same percent", () => {
    const offers: RestaurantTimedOffer[] = [
      { discountPercent: 25, window: { kind: "range", start: "10:00", end: "11:00" } },
      { discountPercent: 25, window: { kind: "range", start: "14:00", end: "16:00" } },
    ]
  ...
  it("three+ extra offers", () => {
    const offers: RestaurantTimedOffer[] = [
      { discountPercent: 35, window: { kind: "range", start: "18:00", end: "22:00" } },
      { discountPercent: 25, window: { kind: "range", start: "12:00", end: "15:00" } },
      { discountPercent: 20, window: { kind: "all-day" } },
    ]
  ...
  it("Melna Bite-like: primary -40% timed window + all-day as extra", () => {
    const offers: RestaurantTimedOffer[] = [
      { discountPercent: 25, window: { kind: "all-day" } },
      { discountPercent: 40, window: { kind: "range", start: "10:00", end: "13:00" } },
    ]
    expect(computeOfferCardCampaign(offers)).toEqual({
      discountLabel: "-40%",
...
describe("restaurantVisibleForPreset", () => {
  ...
  it("morning: venue with only evening timed offers hidden", () => {
    const eveningOnly: RestaurantTimedOffer[] = [
      { discountPercent: 40, window: { kind: "range", start: "18:00", end: "21:00" } },
    ]
